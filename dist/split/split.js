...
function clearResults() { resultUrls.forEach(URL.revokeObjectURL); resultUrls = []; elements.results.hidden = true; elements.resultsList.replaceChildren(); }
...
function addResult(name, data, index) { const url = URL.createObjectURL(new Blob([data], { type: selectedFile.type || 'audio/*' })); resultUrls.push(url); const link = document.createElement('a'); link.className = 'split-download'; link.href = url; link.download = name; link.innerHTML = `<span><b>ส่วน ${index}</b><small>${formatBytes(data.byteLength)}</small></span><em>ดาวน์โหลด ↓</em>`; elements.resultsList.append(link); }
...
elements.input.addEventListener('change', (event) => setFile(event.target.files?.[0])); elements.clear.addEventListener('click', resetFile); elements.parts.addEventListener('input', updateEstimate); elements.split.addEventListener('click', splitAudio); elements.clearResults.addEventListener('click', clearResults);
